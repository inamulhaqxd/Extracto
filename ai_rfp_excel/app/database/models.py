...
class Document(Base):
    ...
    processing_runs_pdf: Mapped[list["ProcessingRun"]] = relationship(back_populates="pdf_document", foreign_keys="ProcessingRun.pdf_document_id")
...
class DocumentTable(Base):
    ...
    merged_cells: Mapped[list | None] = mapped_column(JSONB, nullable=True)
...
class DocumentImage(Base):
    ...
    ocr_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    vision_analysis: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class Workbook(Base):
    ...
    processing_runs_wb: Mapped[list["ProcessingRun"]] = relationship(back_populates="workbook", foreign_keys="ProcessingRun.workbook_id")
...
class WorkbookSheet(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=generate_uuid)
    workbook_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("workbooks.id", ondelete="CASCADE"), nullable=False)
    ...
    merged_cells: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    hidden_rows: Mapped[list | None] = mapped_column(JSONB, nullable=True)
...
class Requirement(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=generate_uuid)
    sheet_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("workbook_sheets.id", ondelete="CASCADE"), nullable=False)
...
class Mapping(Base):
    ...
    compliance_results: Mapped[list["ComplianceResult"]] = relationship(back_populates="mapping", cascade="all, delete-orphan")
...
class ComplianceResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=generate_uuid)
    mapping_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("mappings.id", ondelete="CASCADE"), nullable=False)
    run_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("processing_runs.id", ondelete="CASCADE"), nullable=False)
    status: Mapped[str] = mapped_column(String(50), nullable=False)
    confidence: Mapped[float] = mapped_column(Float, nullable=False)
    ai_reasoning: Mapped[str | None] = mapped_column(Text, nullable=True)
    structured_constraints: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
...
class Evidence(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=generate_uuid)
    compliance_result_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("compliance_results.id", ondelete="CASCADE"), nullable=False)
...
class ProcessingRun(Base):
    ...
    pdf_document_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("documents.id", ondelete="SET NULL"), nullable=True)
    ...
    pdf_document: Mapped["Document | None"] = relationship(foreign_keys=[pdf_document_id], back_populates="processing_runs_pdf")
    workbook: Mapped["Workbook | None"] = relationship(foreign_keys=[workbook_id], back_populates="processing_runs_wb")
