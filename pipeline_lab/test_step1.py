...
def sample_pdf_path() -> Path:
    candidates = [
        ...
        Path("testworkflowfile/refernce.pdf"),
        Path("../testworkflowfile/VERYHIGH_bgp_qos_capacity_spec.pdf"),
        Path("testworkflowfile/VERYHIGH_bgp_qos_capacity_spec.pdf"),
    ]
    for p in candidates:
        if p.exists():
...
def test_automated_validation(sample_pdf_path: Path, tmp_path: Path) -> None:
    ...
    report = validate(json_path, search_terms=["SYS-222C-TN", "Techaccess", "NVMe"])
...
def test_synonym_and_alternative_probing(sample_pdf_path: Path, tmp_path: Path) -> None:
    """Tests that synonyms (e.g. CPU Load -> CPU Usage) and alternative evidence are detected."""
    doc = extract_pdf(sample_pdf_path)
    json_path = tmp_path / "sample_output_syn.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(doc, f, ensure_ascii=False)

    report = validate(json_path, search_terms=["CPU Load", "Xeon 6710E"])
